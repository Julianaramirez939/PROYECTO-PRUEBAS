...
describe('CategoriasService - Pruebas Unitarias', () => {
  ...
  test('crearCategoria debe fallar si el nombre está repetido', async () => {
    await expect(crearCategoria({ nombre: 'Tecnología' }))
      .rejects
      .toThrow('Ya existe una categoría con este nombre');
  });
  ...
  test('actualizarCategoria debe fallar si el nombre está duplicado', async () => {
    ...
    await expect(actualizarCategoria(categoriaId, { nombre: 'Hogar' }))
      .rejects
      .toThrow('Ya existe otra categoría con este nombre');
  });

  test('eliminarCategoria debe fallar si tiene productos asociados', async () => {
    // Simular producto asociado
    ...
    await expect(eliminarCategoria(categoriaId))
      .rejects
      .toThrow('No se puede eliminar la categoría porque tiene productos asociados');
  });
